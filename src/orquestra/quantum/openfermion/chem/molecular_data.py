...
def name_molecule(geometry, basis, multiplicity, charge, description):
    ...
    if not isinstance(geometry, basestring):
        ...
        sorted_info = sorted(
            atom_charge_info, key=lambda atom: periodic_hash_table[atom[0]]
        )
...
class MolecularData(object):
    ...
    def __init__(
        ...
        if (geometry is None) or (basis is None) or (multiplicity is None):
            if filename:
                ...
                raise ValueError(
                    "Geometry, basis, multiplicity must be"
    ...
    def canonical_orbitals(self):
        if self._canonical_orbitals is None:
            ...
            self._canonical_orbitals = (
                data if data is not None and data.dtype.num != 0 else None
            )
    ...
    def save(self):
        ...
        with h5py.File("{}.hdf5".format(tmp_name), "w") as f:
            ...
            d_geom.create_dataset(
                "positions", data=(positions if positions is not None else False)
            )
            # Save basis:
            ...
            f.create_dataset(
                "n_orbitals",
                data=(self.n_orbitals if self.n_orbitals is not None else False),
            )
            f.create_dataset(
            ...
            f.create_dataset(
                "canonical_orbitals",
                data=(
                    self.canonical_orbitals
                    if self.canonical_orbitals is not None
                    else False
                ),
            ...
            f.create_dataset(
                ...
                data=(
                    ...
                    if self.overlap_integrals is not None
                    else False
                ),
            ...
            f.create_dataset(
                "mp2_energy",
                data=(self.mp2_energy if self.mp2_energy is not None else False),
            )
            # Save attributes generated from CISD calculation.
            f.create_dataset(
                "cisd_energy",
                data=(self.cisd_energy if self.cisd_energy is not None else False),
            )
            f.create_dataset(
                "cisd_one_rdm",
            ...
            f.create_dataset(
                "fci_energy",
                data=(self.fci_energy if self.fci_energy is not None else False),
            )
            f.create_dataset(
                "fci_one_rdm",
            ...
            f.create_dataset(
                "ccsd_energy",
                data=(self.ccsd_energy if self.ccsd_energy is not None else False),
            )
            f.create_dataset(
                "ccsd_single_amps",
                data=(
                    self.ccsd_single_amps
                    if self.ccsd_single_amps is not None
                    else False
                ),
            ...
            f.create_dataset(
                "ccsd_double_amps",
                data=(
                    self.ccsd_double_amps
                    if self.ccsd_double_amps is not None
                    else False
                ),
            ...
            f.create_dataset(
                ...
                data=(
                    [numpy.string_(key) for key in key_list]
                    if len(key_list) > 0
                    else False
                ),
            )
    ...
    def load(self):
        ...
        with h5py.File("{}.hdf5".format(self.filename), "r") as f:
            ...
            if data.shape != (()):
                for atom, pos in zip(
                    f["geometry/atoms"][...], f["geometry/positions"][...]
                ):
            ...
            # Load description:
            self.description = (
                f["description"][...].tobytes().decode("utf-8").rstrip("\x00")
            )
            # Load name:
