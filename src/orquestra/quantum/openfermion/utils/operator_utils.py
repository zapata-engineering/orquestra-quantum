    ...
    if isinstance(operator, FermionOperator):
        ...
        for term, coefficient in operator.terms.items():
            conjugate_term = tuple(
                [
                    (tensor_factor, 1 - action)
                    for (tensor_factor, action) in reversed(term)
                ]
            )
...
def is_hermitian(operator):
    ...
    # Handle FermionOperator, and InteractionOperator
    if isinstance(operator, (FermionOperator, InteractionOperator)):
...
def save_operator(
    ...
    if plain_text:
        ...
        with open(file_path, "wb") as f:
            marshal.dump(
                (operator_type, dict(zip(tm.keys(), map(complex, tm.values())))), f
            )
