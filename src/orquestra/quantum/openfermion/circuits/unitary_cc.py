...
def uccsd_generator(single_amplitudes, double_amplitudes, anti_hermitian=True):
    ...
    if isinstance(single_amplitudes, numpy.ndarray) or isinstance(
        double_amplitudes, numpy.ndarray
    ):
...
def uccsd_singlet_get_packed_amplitudes(
    ...
    for p, q in itertools.product(range(n_virtual), range(n_occupied)):
        ...
        doubles_1.append(
            double_amplitudes[virtual_up, occupied_up, virtual_down, occupied_down]
        )
