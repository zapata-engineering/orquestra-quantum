...
from zquantum.qcbm.target import ( get_num_bars_and_stripes_patterns,  get_bars_and_stripes_target_distribution,  bars_and_stripes_zigzag)
...
class TestTarget(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fractions = [ 1., 0.75, 0.5, 0.25, 0.1 ]
        ...
        for method in self.methods:
            for i, grid in enumerate(self.grids):
                for fraction in self.fractions:
                    # When
                    bars_and_stripes_distribution = get_bars_and_stripes_target_distribution(grid[0], grid[1],  fraction=fraction, method=method)
                    
                    # Then
                    self.assertIsInstance(bars_and_stripes_distribution, BitstringDistribution)
                    expected_num_patters = max(int(self.number_of_patterns[i]*fraction), 1)
                    self.assertEqual(len(bars_and_stripes_distribution.distribution_dict.keys()), expected_num_patters)
    ...
    def test_get_bars_and_stripes_target_distribution_unsupported_method(self):
        ...
        # When/Then
        self.assertRaises(RuntimeError, lambda: get_bars_and_stripes_target_distribution(nrows, ncols, fraction=1., method=method))
