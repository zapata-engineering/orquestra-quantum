...
class MolecularDataTest(unittest.TestCase):
        ...
        for atom in range(3):
            ...
            for coordinate in range(3):
                self.assertAlmostEqual(
                    water_geometry[atom][1][coordinate],
    ...
    def test_dummy_save(self):
        ...
        molecule = MolecularData(
            geometry, basis, multiplicity, charge, description, filename
        )
    ...
    def test_active_space(self):
        ...
        self.assertAlmostEqual(
            scipy.linalg.norm(one_body_integrals - self.molecule.one_body_integrals),
            0.0,
        )
        self.assertAlmostEqual(
            scipy.linalg.norm(two_body_integrals - self.molecule.two_body_integrals),
            0.0,
        )
    ...
    def test_antisymint(self):
        ...
        E2bdy = (
            1
            / 2.0
            * np.einsum("ijij", antisymm_spin_orb_tei[:nocc, :nocc, :nocc, :nocc])
        )
        E_test = h2mol.nuclear_repulsion + E1bdy + E2bdy
        self.assertAlmostEqual(E_test, h2mol.hf_energy)
