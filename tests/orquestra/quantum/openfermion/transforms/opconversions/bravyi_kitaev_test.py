...
class BravyiKitaevTransformTest(unittest.TestCase):
    ...
    def test_bravyi_kitaev_transform_sympy(self):
        ...
        sum_lr = bravyi_kitaev(
            FermionOperator(((9, 0),)) * coeff + FermionOperator(((9, 1),)) * coeff,
            n_qubits,
        )
    ...
    def test_bk_n_qubits_too_small(self):
        ...
        with self.assertRaises(ValueError):
            bravyi_kitaev(
                get_interaction_operator(FermionOperator("2^ 3^ 5 0")), n_qubits=4
            )
    ...
    def test_bk_jw_number_operator(self):
        ...
        self.assertAlmostEqual(
            0.0, numpy.amax(numpy.absolute(jw_spectrum - bk_spectrum))
        )
    ...
    def test_bk_jw_majoranas(self):
        ...
        self.assertAlmostEqual(
            0.0, numpy.amax(numpy.absolute(c_spectrum[0] - c_spectrum[1]))
        )
        self.assertAlmostEqual(
            0.0, numpy.amax(numpy.absolute(d_spectrum[0] - d_spectrum[1]))
        )
    ...
    def test_bk_jw_integration_original(self):
        ...
        self.assertAlmostEqual(
            0.0, numpy.amax(numpy.absolute(jw_spectrum - bk_spectrum)), places=5
        )
